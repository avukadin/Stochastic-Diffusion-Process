...
for i=1:paths
    ...
    for j=int:int:T
        ...
        V(k)= (V(k-1))+kappa*(theta-(V(k-1)>0)*V(k-1))/n+sigma*sqrt( (V(k-1)>0)*V(k-1)/n )*z1(k-1)+(1/4)*(1/n)*sigma^2*(z1(k-1)^2-1);
        S(k)=exp(log(S(k-1)) + (1/n)*(r- ( 1/2 )*(V(k-1)>0)*V(k-1)  ) +sqrt( (V(k-1)>0)*V(k-1)/n )*(rho*z1(k-1)+sqrt(1-rho^2)*z2(k-1)) );
        k=k+1;
